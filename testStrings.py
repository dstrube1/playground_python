...
#function doing string stuffs: indexing
def stringF0():
	print()
	string = "string"
	print("string = " + string)
...
#function doing string stuffs: comparison, conversion, and find
def stringF1(s):
	print("testing : '" + s +"'")
...
def stringF4():
	...
	with open(inFileName, inFileMode) as fileIn:
		...
		for lineIn in fileIn:
			#print(lineIn, end='')
			for charIn in lineIn:
				if charIn != '{' and charIn != '}' and not charIn.isalpha() and \
				not charIn.isdigit() and charIn != '"' and charIn != ':' and \
				not charIn.isspace() and charIn != '#' and charIn != '\\' and \
				charIn != ',' and charIn != '.'  and charIn != '[' and charIn != ']' and \
				charIn != '-' and charIn != '@' and charIn != '/' and charIn != '_' \
				and charIn != '(' and charIn != ')' and charIn != '\'' and charIn != '=' \
				and charIn != '~' and charIn != '&' and charIn != '!' and charIn != '$' \
				and charIn != '+' and charIn != '%' and charIn != '?' and charIn != ';' \
				and charIn != '|' and charIn != '<' and charIn != '>' and charIn not in badChars:
					#print(charIn, end='')
					badChars += charIn
			#countIn += 1
			#if countIn > 1000:
			#	break
			with open(outFileName, outFileMode) as fileOut:
				for badChar in badChars:
					if badChar in lineIn:
						lineIn = lineIn.replace(badChar, "")
				fileOut.write(lineIn)
	print()
	#print("length of badChars: " + str(len(badChars)))
